		...
		if(Search != ''){
			...
			$.ajax({
				...
				success:function(results){
					...
					$('.SearchResults').html('<table width="100%"><thead><tr><th>Systeemnaam</th><th>Systeem Url</th></tr></thead><tbody></tbody></table>')

					var data = JSON.parse(results);

					for(var i=0; i<data.length; i++){
						$('.SearchResults table tbody').append('<tr><td>' + data[i].SystemName + '</td><td>' + data[i].SystemUrl + '</td></tr>');
					}
